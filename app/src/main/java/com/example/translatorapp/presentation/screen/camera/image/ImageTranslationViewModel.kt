...
class ImageTranslationViewModel @Inject constructor(
    ...
    fun updateSourceLanguage(language: LanguageCode) {
        ...
        if (state.sourceLanguage == language)
            return
    ...
    fun updateDestinationLanguage(language: LanguageCode) {
        ...
        if (state.destinationLanguage == language)
            return
    ...
    fun onSwapLanguages() {
        ...
        viewModelScope.launch {
            setSourceLanguageUseCase(newSource.code)
            setDestinationLanguageUseCase(newDestination.code)
        }

        retryImageRecognition()
    }

    fun getSourceText(): String = _recognitionState.value
        .recognizedText
        ?.blocks
    ...
    private suspend fun recognizeCurrentImage() {
        ...
        recognizeImageTextUseCase(imageUri = imageUri, sourceLanguage = sourceLanguage)
            .onSuccess { result ->
                _recognitionState.update {
                    it.copy(
    ...
    private suspend fun loadSavedLanguages() {
    ...
    private fun refreshDisplayedBlocks() {
        ...
        _recognitionState.update {
            it.copy(translatedBlocks = blocks)
        }
    }
